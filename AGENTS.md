...
Heavy lifting is delegated to **Sonnet subagents** defined in `.claude/agents/`
(local, gitignored):
...
rebuild those. The `understand-anything` plugin is enabled for this repo
(`.claude/settings.json`).

## Learning mode: OFF

<!-- ON | OFF. When ON, the agent applies the learning ladder automatically:
after each phase, on questions, and when pushing back. See /learn. -->
