...
vim.api.nvim_create_autocmd("VimLeavePre", {
	callback = function()
		...
		pcall(function()
			require("neo-marimo.image").clear_all()
		end)
	end,
})
...
function M.attach(source_bufnr)
	...
	if _attached[filepath] then
		...
		if
			...
			and vim.api.nvim_buf_is_loaded(existing_nb.bufnr)
		then
	...
	nb._on_ws_message = function(msg)
		...
		if _G.neo_marimo_ws_log then
			...
			if f then
				f:write(os.date("[%H:%M:%S] ") .. (op or "?") .. " " .. vim.json.encode(payload) .. "\n")
				f:close()
			end
		end
	...
	vim.api.nvim_create_autocmd("BufWipeout", {
		...
		callback = function()
			...
			if config.options.server and config.options.server.stop_on_close and server.is_running(filepath) then
	...
	if srv_cfg.watch_file ~= false and vim.uv.fs_stat(filepath) then
		watcher.start(filepath, function()
			...
			if f then
				...
				f:close()
				if sync.matches_recent_write(nb, content) then
					return
				end
			end
			...
			if modified then
				local choice = vim.fn.confirm(
					"External change to " .. vim.fn.fnamemodify(filepath, ":t") .. " — discard local edits?",
					"&Yes\n&No",
					2
				)
				if choice ~= 1 then
...
function M.check()
	...
	if result.ok then
		vim.health.ok(
			"Python bridge OK (Python " .. result.python_version .. ", marimo " .. result.marimo_version .. ")"
		)
	else
